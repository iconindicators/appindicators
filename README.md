...
which creates a virtual environment `venv_build`, updates `.pot` / `.po` and creates a `.whl` / `.tar.gz` for `indicatortest` in `release/wheel/dist_indicatortest`. Additional indicators may be appended to the above command.
...
which creates a symbolic link to `indicatorbase.py` for all the indicators.
...
Note for `Debian 12` the version of `PyGObject` is pinned.  Subsequent distributions/versions may require similar treatment.

If the indicator has not previously been installed to `$HOME/.local/venv_indicators`, the icon/locale will be absent.

Some indicators, such as `indicatorlunar`, require additional packages, specified in the `dependencies` field of `pyproject.toml`.  Include those additional packages in the `pip install`.
