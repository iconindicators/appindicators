...
# Frontend to display menu, icon and label.
...
def initialise( indicatorName, iconThemePath ):

    Notify.init( indicatorName )

    indicator = AppIndicator3.Indicator.new( indicatorName, "", AppIndicator3.IndicatorCategory.APPLICATION_STATUS )
    indicator.set_icon_theme_path( iconThemePath )
    indicator.set_status( AppIndicator3.IndicatorStatus.ACTIVE )


def update( indicator, preferences, about ):
    __updateMenu( indicator, preferences, about )
    __updateIconAndLabel( utcNow )


def __updateMenu( indicator, preferences, about ):
    menu = Gtk.Menu()
    __updateMoonMenu( menu )
    __updateSunMenu( menu )
    __updatePlanetsMenu( menu )
    __updateStarsMenu( menu )
#         self.updateCometsMenu( menu )
#         self.updateSatellitesMenu( menu )
    pythonutils.createPreferencesAboutQuitMenuItems( menu, len( menu.get_children() ) > 0, preferences, about, Gtk.main_quit )
    indicator.set_menu( menu )
    menu.show_all()


def __updateIconAndLabel( ephemNow, data, indicatorText ):
    # Substitute tags for values.
    parsedOutput = indicatorText
...
#         lunarPhase = self.getLunarPhase( ephemNow, lunarIlluminationPercentage )
        lunarPhase = self.data[ ( AstronomicalBodyType.Moon, IndicatorLunar.MOON_TAG, IndicatorLunar.DATA_PHASE ) ]
        ...
        lunarIlluminationPercentage = self.data[ ( AstronomicalBodyType.Moon, IndicatorLunar.MOON_TAG, backend.DATA_ILLUMINATION ) ]
