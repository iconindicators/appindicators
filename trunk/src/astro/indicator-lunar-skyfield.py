...
from skyfield.api import earth, JulianDate, mars, moon, now
...
#         self.lastFullMoonNotfication = ephem.Date( "2000/01/01" ) # Set a date way back in the past...
        self.lastFullMoonNotfication = JulianDate( utc = ( 2000, 1, 1 ) ) # Set a date way back in the past...
...
#             ephemNow = ephem.now() # UTC, used in all calculations.  When it comes time to display, conversion to local time takes place.
            ephemNow = now() # UTC, used in all calculations.  When it comes time to display, conversion to local time takes place.
...
# 
# myPosition = earth.topos( "33.8599722 S", "151.2111111 E" )
# alt, az, d = myPosition( now() ).observe( mars ).apparent().altaz()
# print( alt, az )
            m = self.earthLocation( ephemNow ).observe( moon )
#             lunarIlluminationPercentage = int( round( ephem.Moon( self.getCity( ephemNow ) ).phase ) )
...
#         self.getDefaultCity()
        self.earthLocation = earth.topos( "33.8599722 S", "151.2111111 E" )
...
#
# Download latest zip of python-skyfield from
# https://pypi.python.org/pypi/skyfield
# unzip and 
...
# sudo python3 setup.py install
#
# Also downloaded and installed the setup.py of
