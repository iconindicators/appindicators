...
import astro, calendar, copy, datetime, eclipse, json, locale, logging, math, os, pickle, pythonutils, re, satellite, shutil, subprocess, sys, tempfile, threading, time, webbrowser
...
#         self.updateSatellitesMenu( menu )
    ...
    def updateAstronomicalInformation( self ):
        city = self.getCity( None )
#TODO Should it be city.elev or city.elevation?        
        a = astro.Astro( city.lat, city.lon, city.elevation, str( datetime.datetime.utcnow() ) )
        a.calculate()
        self.data = a.getCalculatedData()
