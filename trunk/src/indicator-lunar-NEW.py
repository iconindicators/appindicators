...
#TODO May need to rename given the above tags were renamed.
    CITY_TAG_TRANSLATION = { NAME_TAG_CITY : _( "CITY" ) }
    MOON_TAG_TRANSLATION = { NAME_TAG_MOON : _( "MOON" ) }
    SUN_TAG_TRANSLATION = { NAME_TAG_SUN : _( "SUN" ) }
    ...
    #TODO Do I need the planets each defined as above?  Can't I do a simliar thing to the stars?
    ...
    INDICATOR_TEXT_DEFAULT = "[" + NAME_TAG_MOON + " " + DATA_PHASE + "]"
    ...
    def update( self, scheduled ):
        with threading.Lock():
            ...
            data = { } # Key is a tuple of AstronomicalBodyType, a data tag (upper case( and data tag (upper case).  Value is the data ready for display.

            global _city_data
            data[ ( None, IndicatorLunar.NAME_TAG_CITY, IndicatorLunar.DATA_NAME ) ] = self.cityName
            data[ ( None, IndicatorLunar.NAME_TAG_CITY, IndicatorLunar.DATA_LATITUDE ) ] = str( round( float( _city_data.get( self.cityName )[ 0 ] ), 1 ) )
            data[ ( None, IndicatorLunar.NAME_TAG_CITY, IndicatorLunar.DATA_LONGITUDE ) ] = str( round( float( _city_data.get( self.cityName )[ 1 ] ), 1 ) )
            data[ ( None, IndicatorLunar.NAME_TAG_CITY, IndicatorLunar.DATA_ELEVATION ) ] = str( _city_data.get( self.cityName )[ 2 ] )
...
#             self.updateAstronomicalInformation( ephemNow, self.cometsMagnitude )

            utcNow = datetime.datetime.utcnow()
            result = getAstronomicalInformation( utcNow, logging, self.cityName, self.planets, self.stars,
                                                  [ ], [ ],
                                                  [ ], [ ], 6 )
    ...
    def updateCometsMenu( self, menu ):
        ...
        for comet in self.comets:
            ...
            if key + ( IndicatorLunar.DATA_MESSAGE, ) in self.data and \
               ...
               ): #TODO Show or hide when a message appears...or maybe log?
...
#also a satellite currently rising.
    ...
    def getAstronomicalInformation( utcNow, logging, cityName,
        ...
        calculateMoon( ephemNow, cityName, data, logging )
        calculateSun( ephemNow, cityName, data, logging )
        calculatePlanets( ephemNow, cityName, data, planets )
        calculateStars( ephemNow, cityName, data, stars )
        calculateComets( ephemNow, cityName, data, comets, cometData, cometMagnitude )
        calculateSatellites( ephemNow, cityName, data, satellites, satelliteData )
    ...
    #             nextUpdateInSeconds = ( 60 * 60 )
        ...
        return data # TODO Also return next update date/time.
    ...
    def calculateMoon( ephemNow, cityName, data, logging ):
        calculateCommon( ephemNow, cityName, data, ephem.Moon(), AstronomicalBodyType.Moon, NAME_TAG_MOON )
        key = ( AstronomicalBodyType.Moon, NAME_TAG_MOON )
        moon = ephem.Moon()
        moon.compute( getCity( cityName, ephemNow ) )
        data[ key + ( DATA_BRIGHT_LIMB, ) ] = str( int( round( __getZenithAngleOfBrightLimb( ephemNow, cityName, moon ) ) ) )
        ...
        calculateEclipse( ephemNow, data, AstronomicalBodyType.Moon, NAME_TAG_MOON, logging )
    ...
    def __getZenithAngleOfBrightLimb( ephemNow, cityName, body ): #TODO Verify this still works!
        city = getCity( cityName, ephemNow )
        sun = ephem.Sun( city )
        body.compute( city )
    ...
    #    ephemNow Date/time.
    #    illuminationPercentage The brightness ranging from 0 to 100 inclusive.
    def getLunarPhase( ephemNow, illuminationPercentage ):
    ...
    # http://www.satellite-calculations.com/Satellite/suncalc.htm
    def calculateSun( ephemNow, cityName, data, logging ):
        calculateCommon( ephemNow, cityName, data, ephem.Sun(), AstronomicalBodyType.Sun, NAME_TAG_SUN )
        ...
        calculateEclipse( ephemNow, data, AstronomicalBodyType.Sun, NAME_TAG_SUN, logging )
    
    
    # Calculate next eclipse for either the Sun or Moon.
    def calculateEclipse( ephemNow, data, astronomicalBodyType, dataTag, logging ):
    ...
    # http://www.ga.gov.au/earth-monitoring/astronomical-information/planet-rise-and-set-information.html
    def calculatePlanets( ephemNow, cityName, data, planets ):
        for planet in planets:
            planetObject = getattr( ephem, planet.title() )()
            calculateCommon( ephemNow, cityName, data, planetObject, AstronomicalBodyType.Planet, planet )
    
    
    # http://aa.usno.navy.mil/data/docs/mrst.php
    def calculateStars( ephemNow, cityName, data, stars ):
        for star in stars:
            starObject = ephem.star( star.title() )
            calculateCommon( ephemNow, cityName, data, starObject, AstronomicalBodyType.Star, star )
    ...
    def calculateComets( ephemNow, cityName, data, comets, cometData, cometMagnitude ):
        for key in comets:
            if key in cometData:
                comet = ephem.readdb( cometData[ key ] )
                comet.compute( getCity( cityName, ephemNow ) )
                if math.isnan( comet.earth_distance ) or math.isnan( comet.phase ) or math.isnan( comet.size ) or math.isnan( comet.sun_distance ): # Have found the data file may contain ***** in lieu of actual data!
                    ...
                    if float( comet.mag ) <= float( cometMagnitude ):
                        calculateCommon( ephemNow, cityName, data, comet, AstronomicalBodyType.Comet, key )
            else:
                data[ ( AstronomicalBodyType.Comet, key, DATA_MESSAGE ) ] = MESSAGE_DATA_NO_DATA
    
    
    def calculateCommon( ephemNow, cityName, data, body, astronomicalBodyType, nameTag ):
        ...
        body.compute( getCity( cityName, ephemNow ) ) # Need to recompute the body otherwise the azimuth/altitude are incorrectly calculated.
        data[ key + ( DATA_AZIMUTH, ) ] = str( body.az )
        data[ key + ( DATA_ALTITUDE, ) ] = str( body.alt )
    
    
    #TODO Probabaly not needed.
    #     def __hideBody( data, astronomicalBodyType, nameTag, hideIfNeverUp ):
    #         key = ( astronomicalBodyType, nameTag, DATA_MESSAGE )
    #         return \
    #             key in data and \
    #             data[ key ] == MESSAGE_BODY_NEVER_UP and \
    #             hideIfNeverUp
    ...
    def calculateSatellites( ephemNow, cityName, data, satellites, satelliteData ):
        for key in satellites:
            if key in satelliteData:
                calculateNextSatellitePass( ephemNow, key, satelliteData[ key ] )
            else:
                data[ ( AstronomicalBodyType.Satellite, " ".join( key ), DATA_MESSAGE ) ] = MESSAGE_DATA_NO_DATA
    
    
    def calculateNextSatellitePass( ephemNow, cityName, data, key, satelliteTLE ):
        ...
        while currentDateTime < endDateTime:
            city = getCity( cityName, currentDateTime )
            ...
            if not isSatellitePassValid( nextPass ):
            ...
            if nextPass[ 0 ] > nextPass[ 4 ]: # The rise time is after set time, so the satellite is currently passing.
                setTime = nextPass[ 4 ]
                nextPass = calculateSatellitePassForRisingPriorToNow( currentDateTime, satelliteTLE )
                if nextPass is None:
            ...
            passIsVisible = isSatellitePassVisible( nextPass[ 2 ], cityName, satellite )
    ...
    def calculateSatellitePassForRisingPriorToNow( ephemNow, cityName, satelliteTLE ):
    ...
    def isSatellitePassVisible( passDateTime, cityName, satellite ):
        city = __getCity( cityName, passDateTime )
        city.pressure = 0
        city.horizon = "-0:34"
    ...
    # This allows the user to see the rise/set time for the current pass as it is happening.
    # When the pass completes and an update occurs, the rise/set for the next pass will be displayed.
    def updateSatellitesORIG( self, ephemNow ):
    ...
    def calculateNextSatellitePassORIG( self, ephemNow, key, satelliteTLE ):
        key = ( AstronomicalBodyType.Satellite, " ".join( key ) )
        currentDateTime = ephemNow
        endDateTime = ephem.Date( ephemNow + ephem.hour * 24 * 2 ) # Stop looking for passes 2 days from ephemNow.
        message = None
    ...
    def calculateSatellitePassForRisingPriorToNowORIG( self, ephemNow, satelliteTLE ):
        currentDateTime = ephem.Date( ephemNow - ephem.minute ) # Start looking from one minute ago.
        endDateTime = ephem.Date( ephemNow - ephem.hour * 1 ) # Only look back an hour for the rise time (then just give up).
        nextPass = None
        while currentDateTime > endDateTime:
    ...
    def isSatellitePassValidORIG( self, satellitePass ):
        return \
            satellitePass is not None and \
            len( satellitePass ) == 6 and \
            satellitePass[ 0 ] is not None and \
    ...
    #    http://www.celestrak.com/columns/v03n01
    #    http://stackoverflow.com/questions/19739831/is-there-any-way-to-calculate-the-visual-magnitude-of-a-satellite-iss
    def isSatellitePassVisibleORIG( self, satellite, passDateTime ):
