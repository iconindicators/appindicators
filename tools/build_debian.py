...
#TODO What to do with this script when we go all in on PYPI?
# Maybe rewrite to use the orig.tar.gz from the buildWheel.py
# which should remove a lot of stuff (no icon/mo building).
...
# Might be of help in creating a postrm for indicator lunar:
...
if __name__ == "__main__":
    ...
    scriptPathAndName = "tools/build_debian.py"
    if Path( scriptPathAndName ).exists():
        ...
        print(
            ...
            f"For example:\n"
            f"\tpython3 { scriptPathAndName } release indicatorfortune" )
