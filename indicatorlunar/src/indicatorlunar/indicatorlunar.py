...
class IndicatorLunar( IndicatorBase ):
    ...
    def __init__( self ):
        ...
        # Dictionary for currently calculated astronomical data.
        ...
        # Dictionary for previously calculated astronomical data, used for
        # satellite transits.
        ...
        self._initialise_download_counts_and_cache_date_times()
    ...
    def _initialise_download_counts_and_cache_date_times( self ):
    ...
    def update(
        ...
        self.set_label_or_tooltip( self._process_tags() )

        self.update_icon()

        if self.show_werewolf_warning:
            self._notification_full_moon()

        if self.show_satellite_notification:
            self.notification_satellites()

        return self._get_next_update_time_in_seconds()
    ...
    def update_data(
        ...
        utc_now ):
        '''
        Download data for comets, minor planets and satellites.
        '''
    ...
    def _process_tags( self ):
        ...
        # Remaining tags are dealt with at the end.
    ...
    def _get_next_update_time_in_seconds( self ):
        date_times = [ ]
        ...
        # Ensure updates don't happen more frequently than every minute.
        utc_now_plus_one_minute = utc_now + datetime.timedelta( minutes = 1 )

        # Do an update at most twenty minutes from now; keeps the moon icon
        # and data fresh.
        next_update_time = utc_now + datetime.timedelta( minutes = 20 )
    ...
    def _notification_full_moon( self ):
        utc_now = datetime.datetime.now( datetime.timezone.utc )
        key = (
            IndicatorLunar.astro_backend.BodyType.MOON,
            IndicatorLunar.astro_backend.NAME_TAG_MOON )
        ...
        if show_notification:
            ...
            self.show_notification(
                ...
                icon = self._create_full_moon_icon() )

            self.last_full_moon_notfication = utc_now


    def _create_full_moon_icon( self ):
        return (
            self.write_cache_text(
                self.get_svg_icon_text(
                    IndicatorLunar.astro_backend.LUNAR_PHASE_FULL_MOON,
    ...
    def _update_menu_sun(
        ...
        if updated:
            ...
            equinox_before_solsitce = (
                ...
                <
                self.data[ key + ( IndicatorLunar.astro_backend.DATA_TAG_SOLSTICE, ) ] )

            if equinox_before_solsitce:
                self.create_and_append_menuitem(
                    submenu,
                    equinox_label,
    ...
    def _update_menu_planets_minor_planets_comets_stars(
        self,
        menu,
        menu_label,
        bodies,
    ...
    def _update_menu_common(
        ...
        key = ( body_type, name_tag )
        appended = False
        if key + ( IndicatorLunar.astro_backend.DATA_TAG_RISE_DATE_TIME, ) in self.data:
            body_will_rise = (
                self.data[ key + ( IndicatorLunar.astro_backend.DATA_TAG_RISE_DATE_TIME, ) ]
                <
                self.data[ key + ( IndicatorLunar.astro_backend.DATA_TAG_SET_DATE_TIME, ) ] )
        ...
        elif key + ( IndicatorLunar.astro_backend.DATA_TAG_AZIMUTH, ) in self.data:
            # Body is 'always up'.
            self._update_menuitems_rise_azimuth_altitude_set(
                menu,
                key,
    ...
    def update_menu_satellites(
        ...
        for number in self.satellites:
            key = ( IndicatorLunar.astro_backend.BodyType.SATELLITE, number )
            if key + ( IndicatorLunar.astro_backend.DATA_TAG_RISE_DATE_TIME, ) in self.data:
                satellite_will_rise_within_next_five_minutes = (
                    self.data[ key + ( IndicatorLunar.astro_backend.DATA_TAG_RISE_DATE_TIME, ) ]
                    <
                    utc_now_plus_five_minutes )

                if satellite_will_rise_within_next_five_minutes:
                    satellites.append( [
                        number,
                        self.satellite_general_perturbation_data[ number ].get_name(),
                        self.data[ key + ( IndicatorLunar.astro_backend.DATA_TAG_RISE_DATE_TIME, ) ],
    ...
    def convert_start_hour_and_end_hour_to_date_time_in_utc(
        ...
        start_hour_as_datetime_in_utc = (
            now.replace( hour = start_hour ).astimezone( datetime.timezone.utc ) )

        end_hour_as_datetime_in_utc = (
            now.replace( hour = end_hour ).astimezone( datetime.timezone.utc ) )
    ...
    def get_svg_icon_text(
        ...
        if is_full_or_new:
            ...
            # First/Third Quarter or
            # Waning/Waxing Crescent or
            ...
            if is_first_quarter_or_third_quarter:
                ...
                body += (
                    ' A ' + str( radius ) + ' ' +
                    str( radius * ( 50 - illumination_percentage ) / 50 ) +
                    ' 0 0 0 ' + str( width / 2 - radius ) + ' ' + str( height / 2 )
                    + '"' )

            else: # Waning/Waxing Gibbous
                body += (
                    ' A ' + str( radius ) + ' ' +
                    str( radius * ( illumination_percentage - 50 ) / 50 ) +
                    ' 0 0 1 ' + str( width / 2 - radius ) + ' ' + str( height / 2 )
                    + '"' )
    ...
    def on_preferences(
        ...
        self._initialise_display_tags_store( display_tags_store )

        indicator_text.set_text(
            self._translate_text_using_tags(
                display_tags_store,
                True,
        ...
        treeview, scrolledwindow = (
            self.create_treeview_within_scrolledwindow(
                ...
                rowactivatedfunctionandarguments = (
                    self._on_tags_values_double_click,
                    column_model_translated_tag,
                    indicator_text ) ) )
        ...
        scrolledwindow_planets = (
            self._create_natural_body_treeview_within_scrolledwindow(
                planet_store,
                _( "Planets" ),
                _(
        ...
        show_satellite_notification_checkbox, satellite_notification_summary_text, satellite_notification_message_text = (
        ...
        city.connect(
            "changed", self._on_city_changed, latitude, longitude, elevation )
        ...
        while True:
            ...
            satellite_limit_start_greater_than_or_equal_to_end = (
                spinner_satellite_limit_start.get_value_as_int()
                >=
                spinner_satellite_limit_end.get_value_as_int() )

            if satellite_limit_start_greater_than_or_equal_to_end:
            ...
            self.satellite_notification_message = (
                self._translate_text_using_tags(
                    ...
                    self.get_textview_text(
                        satellite_notification_message_text ) ) )
    ...
    def _initialise_display_tags_store(
        self,
        display_tags_store ):
        ...
        for item in items:
            ...
            if azimuth_in_data:
                ...
                for data_tag in data_tags:
                    ...
                    if key in self.data:
                        value = self._format_data( data_tag, self.data[ key ] )
            ...
            for body_tag in body_tags:
                ...
                if azimuth_in_data:
                    ...
                    for data_tag in data_tags:
                        ...
                        if key in self.data:
                            value = (
                                self._format_data( data_tag, self.data[ key ] ) )
                            display_tags_store.append( [
                                body_tag + " " + data_tag, translated_tag, value ] )
    ...
    def _translate_text_using_tags(
        self,
        tags_list_store,
        original_to_local,
        text ):
    ...
    def _on_tags_values_double_click(
        self,
        tree,
        path,
        treeviewcolumn,
    ...
    def _create_natural_body_treeview_within_scrolledwindow(
        self,
        treemodel,
        title,
        tooltip_text,
        ...
        test = (
            self.create_button(
                ...
                clicked_functionandarguments = (
                    self._on_test_notification_clicked,
                    summary_text_entry,
                    message_text_view,
                    is_moon_notification ) ) )
    ...
    def _on_test_notification_clicked(
        self,
        button,
        summary_entry,
        message_text_view,
    ...
    def get_default_city( self ):
        '''
        Get the default city based on the system's timezone.
        '''
