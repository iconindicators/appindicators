...
class DataProviderOrbitalElement( DataProvider ):
    ...
    def download(
        ...
        if not ( is_comet or is_minor_planet ):
            raise ValueError(
    ...
    def _download_from_lowell_minor_planet_services(
        ...
        if json_:
            downloaded = True
            DataProviderOrbitalElement._process_json_and_write(
                json_,
                orbital_element_data_type,
                filename )
    ...
    def _process_json_and_write(
        ...
        if xephem_minor_planet:
            append_function = (
                getattr(
                    DataProviderOrbitalElement,
                    "_append_minor_planet_xephem" ) )

        else:
            append_function = (
                getattr(
                    DataProviderOrbitalElement,
                    "_append_minor_planet_skyfield" ) )
        ...
        for minor_planet in minor_planets:
            ...
            # XEphem has three formats for minor planets based on the value
            # of the eccentricity ( < 1, == 1, > 1 )
            #   https://xephem.github.io/XEphem/Site/help/xephem.html#mozTocId468501
            #
            # When the eccentricity is >= 1, the format is the same and
            # requires date of epoch of perihelion, which does not appear
            # to be present in the Lowell data.
            #
    ...
    @staticmethod
    def _append_minor_planet_skyfield(
        minor_planet,
        designation,
        absolute_magnitude,
        slope_parameter,
    ...
    @staticmethod
    def _append_minor_planet_xephem(
        minor_planet,
        designation,
        absolute_magnitude,
        slope_parameter,
    ...
    def _download_from_comet_observation_database(
        ...
        url = (
            ...
            "&is-active=true" +
            "&is-observed=true" +
            f"&cur-mag={ str( int( apparent_magnitude_maximum ) ) }" )
    ...
    def load(
        ...
        if not ( is_skyfield_data or is_xephem_data ):
            raise ValueError(
                f"Unknown data type: { str( orbital_element_data_type ) }" )
        ...
        # successful.
        if is_skyfield_data:
            download_function = "_load_skyfield"

        else:
            download_function = "_load_xephem"
